...
assert.ok(/more\.parentNode !== ctrl\) ctrl\.appendChild\(more\)/.test(code), "the control row is reused, not rebuilt, when it survives");
	assert.ok(/if \(remaining > 0\) \{\n\t\t\t\t\t\tconst label = "\\u5c55\\u5f00\\u5176\\u4f59 " \+ Math\.min\(WS_PAGE, remaining\) \+ " \\u4e2a\\u4f1a\\u8bdd";/.test(code), "the collapsed page shows our own 展开其余 5 个会话 control when rows remain");
