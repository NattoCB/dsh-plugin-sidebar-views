window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function rowEl(s, current, now, wsOf) {
		...
		function renderList() {
			...
			for (const g of groups) {
				...
				if (collapsed === false) {
					for (const s of g.items) listDiv.appendChild(rowEl(s, current, now, wsOf));
				}
			}
